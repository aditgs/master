...
class Siakad_pt_jabatan_model extends CI_Model {
    ...
    function get_all($limit, $uri) {
        ...
        $result = $this->db->get('siakad_pt_jabatan', $limit, $uri);
    ...
    function ceknomornull(){
        ...
        $this->db->order_by('id_siakad_pt_jabatan','ASC');
        $this->db->limit(1);

        $result=$this->db->get('siakad_pt_jabatan');
    ...
    function get_last(){
        ...
        $this->db->select('*'); //faktur
        $this->db->order_by('id_siakad_pt_jabatan','DESC');
        $this->db->limit(1);
    ...
    function gettotaldetail($faktur){
        ...
        $this->db->from('siakad_pt_jabatan');
    ...
    function get_one($id_siakad_pt_jabatan) {
        $this->db->where('id_siakad_pt_jabatan', $id_siakad_pt_jabatan);
        $result = $this->db->get('siakad_pt_jabatan');
        if ($result->num_rows() == 1) {
            return $result->row_array();
        } else {
    ...
    function __save(){
       ...
       $data = array(
            ...
            'periode_jbt_awal' => $this->input->post('periode_jbt_awal', TRUE),
           
            'periode_jbt_akhir' => $this->input->post('periode_jbt_akhir', TRUE),
    ...
    function savesiakad_pt_jabatan($data){
        $this->db->insert('siakad_pt_jabatan',$data);
    }
    function save_detail($data){
        $this->db->insert('siakad_pt_jabatan_detail',$data);
    }
    function upddel_detail($id_siakad_pt_jabatan=null) {
    ...
    function updatebyid($id_siakad_pt_jabatan,$data){
        $this->db->where('id_siakad_pt_jabatan', $id_siakad_pt_jabatan);
        $this->db->update('siakad_pt_jabatan', $data);
    }
    function update($id_siakad_pt_jabatan) {
        $data = array(
        'id_siakad_pt_jabatan' => $this->input->post('id_siakad_pt_jabatan',TRUE),
       'id_siakad_pt_struk_jbt' => $this->input->post('id_siakad_pt_struk_jbt', TRUE),
    ...
    function delete($id_siakad_pt_jabatan) {
        $this->db->where('id_siakad_pt_jabatan', $id_siakad_pt_jabatan);
        $this->db->delete('siakad_pt_jabatan'); 
       
    }
    function delete_detail($id=null) {
        $this->db->where('id_detail', $id);
        $this->db->delete('siakad_pt_jabatan_detail'); 
    ...
    function get_dropdown_array($value){
        $result = array();
        $array_keys_values = $this->db->query('select id_siakad_pt_jabatan, '.$value.' from siakad_pt_jabatan order by id_siakad_pt_jabatan asc');
        $result[0]="-- Pilih Urutan id_siakad_pt_jabatan --";
        foreach ($array_keys_values->result() as $row)
        {
            $result[$row->id_siakad_pt_jabatan]= $row->$value;
        }
        return $result;
    }
