...
class mk_paket_det extends MX_Controller {
    ...
    function __construct() {
        ...
        $this->session->set_userdata('lihat','mk_paket_det');
    ...
    public function index() {
        $this->template->set_title('Kelola MK Paket Detail');
        $this->template->add_js('var baseurl="'.base_url().'mk_paket_det/";','embed');  
        $this->template->load_view('siakad_mk_paket_det_view',array(
            'view'=>'',
            'title'=>'Kelola Data MK Paket Detail',
            'subtitle'=>'Pengelolaan MK Paket Detail',
            'breadcrumb'=>array(
            'MK Paket Detail'),
        ));
    }
    ...
    public function getdatatables(){
        if($this->isadmin()==1):
            ...
            $this->datatables->add_column('edit',"<div class='btn-group'>
                <a data-toggle='modal' href='#modal-id' data-load-remote='".base_url('mk_paket_det/getone/$1/')."' data-remote-target='#modal-id .modal-body' class='btn btn-info btn-xs'><i class='fa fa-info-circle'></i> </a>
            ...
            $this->datatables->select('id_siakad_mk_paket_det,kode_mk,id_siakad_mk_paket,')
                            ->from('siakad_mk_paket_det');
            $this->datatables->add_column('edit',"<div class='btn-group'>
                <a data-toggle='modal' href='#modal-id' data-load-remote='".base_url('mk_paket_det/getone/$1/')."' data-remote-target='#modal-id .modal-body' class='btn btn-info btn-xs'><i class='fa fa-info-circle'></i> </a></div>" , 'id_siakad_mk_paket_det');
            $this->datatables->unset_column('id_siakad_mk_paket_det');
        endif;
