...
class prodi_log extends MX_Controller {
    ...
    function __construct() {
        ...
        $this->session->set_userdata('lihat','prodi_log');
    ...
    public function index() {
        $this->template->set_title('Kelola Prodi Log');
        $this->template->add_js('var baseurl="'.base_url().'prodi_log/";','embed');  
        $this->template->load_view('siakad_prodi_log_view',array(
            'view'=>'',
            'title'=>'Kelola Data Prodi Log',
            'subtitle'=>'Pengelolaan Prodi Log',
            'breadcrumb'=>array(
            'Prodi Log'),
        ));
    }
    ...
    public function getdatatables(){
        if($this->isadmin()==1):
            ...
            $this->datatables->add_column('edit',"<div class='btn-group'>
                <a data-toggle='modal' href='#modal-id' data-load-remote='".base_url('prodi_log/getone/$1/')."' data-remote-target='#modal-id .modal-body' class='btn btn-info btn-xs'><i class='fa fa-info-circle'></i> </a>
            ...
            $this->datatables->select('id_siakad_prodi_log,kode_prodi,user_prodi,pass_prodi,email_prodi,style_prodi,')
                            ->from('siakad_prodi_log');
            $this->datatables->add_column('edit',"<div class='btn-group'>
                <a data-toggle='modal' href='#modal-id' data-load-remote='".base_url('prodi_log/getone/$1/')."' data-remote-target='#modal-id .modal-body' class='btn btn-info btn-xs'><i class='fa fa-info-circle'></i> </a></div>" , 'id_siakad_prodi_log');
            $this->datatables->unset_column('id_siakad_prodi_log');
        endif;
