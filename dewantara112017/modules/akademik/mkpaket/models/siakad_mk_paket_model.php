...
class Siakad_mk_paket_model extends CI_Model {
    ...
    function get_all($limit, $uri) {
        ...
        $result = $this->db->get('siakad_mk_paket', $limit, $uri);
    ...
    function ceknomornull(){
        ...
        $this->db->order_by('id_siakad_mk_paket','ASC');
        $this->db->limit(1);

        $result=$this->db->get('siakad_mk_paket');
    ...
    function get_last(){
        ...
        $this->db->select('*'); //faktur
        $this->db->order_by('id_siakad_mk_paket','DESC');
        $this->db->limit(1);
    ...
    function get_one($id_siakad_mk_paket) {
        $this->db->where('id_siakad_mk_paket', $id_siakad_mk_paket);
        $result = $this->db->get('siakad_mk_paket');
        if ($result->num_rows() == 1) {
            return $result->row_array();
        } else {
    ...
    function __save(){
       ...
       $data = array(
            ...
            'nm_paket_mk' => $this->input->post('nm_paket_mk', TRUE),
           
            'status_paket_mk' => $this->input->post('status_paket_mk', TRUE),
    ...
    function savesiakad_mk_paket($data){
        $this->db->insert('siakad_mk_paket',$data);
    }
    function save_detail($data){
        $this->db->insert('siakad_mk_paket_detail',$data);
    }
    function upddel_detail($id_siakad_mk_paket=null) {
    ...
    function updatebyid($id_siakad_mk_paket,$data){
        $this->db->where('id_siakad_mk_paket', $id_siakad_mk_paket);
        $this->db->update('siakad_mk_paket', $data);
    }
    function update($id_siakad_mk_paket) {
        $data = array(
    ...
    function delete($id_siakad_mk_paket) {
        $this->db->where('id_siakad_mk_paket', $id_siakad_mk_paket);
        $this->db->delete('siakad_mk_paket'); 
       
    }
    function delete_detail($id=null) {
        $this->db->where('id_detail', $id);
        $this->db->delete('siakad_mk_paket_detail'); 
    ...
    function get_dropdown_array($value){
        $result = array();
        $array_keys_values = $this->db->query('select id_siakad_mk_paket, '.$value.' from siakad_mk_paket order by id_siakad_mk_paket asc');
        $result[0]="-- Pilih Urutan id_siakad_mk_paket --";
        foreach ($array_keys_values->result() as $row)
        {
            $result[$row->id_siakad_mk_paket]= $row->$value;
        }
        return $result;
    }
