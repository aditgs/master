...
class Mhsadm extends MX_Controller {

    function __construct() {
        parent::__construct();
          
        //Load IgnitedDatatables Library
        $this->load->model('siakad_mhs_adm_model','mhsadmdb',TRUE);
        $this->session->set_userdata('lihat','mhsadm');
    ...
    function __getnewfaktur(){
        ...
        $null=$this->mhsadmdb->ceknomornull();
        // print_r($null);
        if($null!=null||!empty($null)){
            ...
            $faktur=$this->mhsadmdb->genfaktur();
    ...
    public function getdatatables(){
            ...
            $this->datatables->select('id,nim_mhs,kode_akademik,')
                            ->from('siakad_mhs_adm');
            $this->datatables->add_column('edit',"<div class='btn-group'>
                <a data-toggle='modal' href='#modal-id' data-load-remote='".base_url('mhsadm/getone/$1/')."' data-remote-target='#modal-id .modal-body' class='btn btn-info btn-xs'><i class='fa fa-info-circle'></i> </a>
    ...
    function enkrip(){
        ...
        // echo $this->session->userdata('mhsadm');
    }
    function isadmin(){
    ...
    function forms(){   
        ...
        $this->load->view('siakad_mhs_adm_form_inside');
           
    }

    public function get($id=null){
        if($id!==null){
            echo json_encode($this->mhsadmdb->get_one($id));
        }
    }
    function tables(){
        $this->load->view('siakad_mhs_adm_data');
    }

    function getone($id=null){
        if($id!==null){
            $data=$this->mhsadmdb->get_one($id);
            $jml=count($data);
            // print_r($jml);
            // print_r($data);
            $div='';
    ...
    public function submit(){
        if ($this->input->post('ajax')){
          if ($this->input->post('id')){
            $this->mhsadmdb->update($this->input->post('id'));
          }else{
            //$this->mhsadmdb->save();
            $this->mhsadmdb->saveas();
          }
    ...
    public function delete(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id'])){
                $this->mhsadmdb->delete($this->input->post('id'));
    ...
    public function delete_detail(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id'])){
                $this->mhsadmdb->upddel_detail($this->input->post('id'));
                $this->session->set_flashdata('notif','Succeed, Data Has Deleted');
     ...
     public function delete_detailxx(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id'])){
                $this->mhsadmdb->delete_detail($this->input->post('id'));
                $this->session->set_flashdata('notif','Succeed, Data Has Deleted');
            }else {
                $this->session->set_flashdata('notif', 'Failed! No Data Deleted');
            }
