...
class Siakad_pt_struk_jbt_model extends CI_Model {
    ...
    function get_all($limit, $uri) {
        ...
        $result = $this->db->get('siakad_pt_struk_jbt', $limit, $uri);
    ...
    function ceknomornull(){
        ...
        $this->db->order_by('id_siakad_pt_struk_jbt','ASC');
        $this->db->limit(1);

        $result=$this->db->get('siakad_pt_struk_jbt');
    ...
    function get_last(){
        ...
        $this->db->select('*'); //faktur
        $this->db->order_by('id_siakad_pt_struk_jbt','DESC');
        $this->db->limit(1);
    ...
    function gettotaldetail($faktur){
        ...
        $this->db->from('siakad_pt_struk_jbt');
    ...
    function get_one($id_siakad_pt_struk_jbt) {
        $this->db->where('id_siakad_pt_struk_jbt', $id_siakad_pt_struk_jbt);
        $result = $this->db->get('siakad_pt_struk_jbt');
        if ($result->num_rows() == 1) {
            return $result->row_array();
        } else {
    ...
    function __save(){
       ...
       $data = array(
            ...
            'nm_struktur_jabatan' => $this->input->post('nm_struktur_jabatan', TRUE),
           
            'def_pos_laporan' => $this->input->post('def_pos_laporan', TRUE),
    ...
    function savesiakad_pt_struk_jbt($data){
        $this->db->insert('siakad_pt_struk_jbt',$data);
    }
    function save_detail($data){
        $this->db->insert('siakad_pt_struk_jbt_detail',$data);
    }
    function upddel_detail($id_siakad_pt_struk_jbt=null) {
    ...
    function updatebyid($id_siakad_pt_struk_jbt,$data){
        $this->db->where('id_siakad_pt_struk_jbt', $id_siakad_pt_struk_jbt);
        $this->db->update('siakad_pt_struk_jbt', $data);
    }
    function update($id_siakad_pt_struk_jbt) {
        $data = array(
        'id_siakad_pt_struk_jbt' => $this->input->post('id_siakad_pt_struk_jbt',TRUE),
       'nm_struktur_jabatan' => $this->input->post('nm_struktur_jabatan', TRUE),
    ...
    function delete($id_siakad_pt_struk_jbt) {
        $this->db->where('id_siakad_pt_struk_jbt', $id_siakad_pt_struk_jbt);
        $this->db->delete('siakad_pt_struk_jbt'); 
       
    }
    function delete_detail($id=null) {
        $this->db->where('id_detail', $id);
        $this->db->delete('siakad_pt_struk_jbt_detail'); 
    ...
    function get_dropdown_array($value){
        $result = array();
        $array_keys_values = $this->db->query('select id_siakad_pt_struk_jbt, '.$value.' from siakad_pt_struk_jbt order by id_siakad_pt_struk_jbt asc');
        $result[0]="-- Pilih Urutan id_siakad_pt_struk_jbt --";
        foreach ($array_keys_values->result() as $row)
        {
            $result[$row->id_siakad_pt_struk_jbt]= $row->$value;
        }
        return $result;
    }
