...
class mk extends MX_Controller {
    ...
    function __construct() {
        ...
        $this->session->set_userdata('lihat','mk');
    ...
    public function index() {
        $this->template->set_title('Kelola Mata Kuliah');
        $this->template->add_js('var baseurl="'.base_url().'mk/";','embed');  
        $this->template->load_view('siakad_mk_view',array(
            'view'=>'',
            'title'=>'Kelola Data MK',
            'subtitle'=>'Pengelolaan MK',
            'breadcrumb'=>array(
            'MK'),
        ));
    }
    public function data() {
        $this->template->set_title('Kelola MK');
        $this->template->add_js('var baseurl="'.base_url().'mk/";','embed');  
        $this->template->load_view('siakad_mk_view',array(
            'view'=>'Siakad_mk_data',
            'title'=>'Kelola Data MK',
    ...
    public function getdatatables(){
        if($this->isadmin()==1):
            ...
            $this->datatables->add_column('edit',"<div class='btn-group'>
                <a data-toggle='modal' href='#modal-id' data-load-remote='".base_url('mk/getone/$1/')."' data-remote-target='#modal-id .modal-body' class='btn btn-info btn-xs'><i class='fa fa-info-circle'></i> </a>
            ...
            $this->datatables->select('kode_mk,id_siakad_kurikulum,kode_prodi,nm_mk,jns_mk,kurikulum_mk,kelompok_mk,sks_mk,sks_tatapmuka,sks_praktikum,min_mk_lulus,status_mk,upload_silabus_mk,upload_sap_mk,upload_bahan_mk,upload_diktat_mk,')
                            ->from('siakad_mk');
            $this->datatables->add_column('edit',"<div class='btn-group'>
                <a data-toggle='modal' href='#modal-id' data-load-remote='".base_url('mk/getone/$1/')."' data-remote-target='#modal-id .modal-body' class='btn btn-info btn-xs'><i class='fa fa-info-circle'></i> </a></div>" , 'kode_mk');
            $this->datatables->unset_column('kode_mk');
        endif;
