...
class Siakad_mk_syarat_model extends CI_Model {
    ...
    function get_all($limit, $uri) {
        ...
        $result = $this->db->get('siakad_mk_syarat', $limit, $uri);
    ...
    function ceknomornull(){
        ...
        $this->db->order_by('id_siakad_mk_syarat','ASC');
        $this->db->limit(1);

        $result=$this->db->get('siakad_mk_syarat');
    ...
    function get_last(){
        ...
        $this->db->select('*'); //faktur
        $this->db->order_by('id_siakad_mk_syarat','DESC');
        $this->db->limit(1);
    ...
    function get_one($id_siakad_mk_syarat) {
        $this->db->where('id_siakad_mk_syarat', $id_siakad_mk_syarat);
        $result = $this->db->get('siakad_mk_syarat');
        if ($result->num_rows() == 1) {
            return $result->row_array();
        } else {
    ...
    function __save(){
       ...
       $data = array(
            ...
            'kode_mk_main' => $this->input->post('kode_mk_main', TRUE),
           
            'kode_mk_syarat' => $this->input->post('kode_mk_syarat', TRUE),
           
            'nil_mk_syarat' => $this->input->post('nil_mk_syarat', TRUE),
           
            'nil_angka_mk_syarat' => $this->input->post('nil_angka_mk_syarat', TRUE),
    ...
    function savesiakad_mk_syarat($data){
        $this->db->insert('siakad_mk_syarat',$data);
    }
    function save_detail($data){
        $this->db->insert('siakad_mk_syarat_detail',$data);
    }
    function upddel_detail($id_siakad_mk_syarat=null) {
    ...
    function updatebyid($id_siakad_mk_syarat,$data){
        $this->db->where('id_siakad_mk_syarat', $id_siakad_mk_syarat);
        $this->db->update('siakad_mk_syarat', $data);
    }
    function update($id_siakad_mk_syarat) {
        $data = array(
        'id_siakad_mk_syarat' => $this->input->post('id_siakad_mk_syarat',TRUE),
       'kode_prodi' => $this->input->post('kode_prodi', TRUE),
    ...
    function delete($id_siakad_mk_syarat) {
        $this->db->where('id_siakad_mk_syarat', $id_siakad_mk_syarat);
        $this->db->delete('siakad_mk_syarat'); 
       
    }
    function delete_detail($id=null) {
        $this->db->where('id_detail', $id);
        $this->db->delete('siakad_mk_syarat_detail'); 
    ...
    function get_dropdown_array($value){
        $result = array();
        $array_keys_values = $this->db->query('select id_siakad_mk_syarat, '.$value.' from siakad_mk_syarat order by id_siakad_mk_syarat asc');
        $result[0]="-- Pilih Urutan id_siakad_mk_syarat --";
        foreach ($array_keys_values->result() as $row)
        {
            $result[$row->id_siakad_mk_syarat]= $row->$value;
        }
        return $result;
    }
