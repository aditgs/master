...
class Siakad_jadwal_ampu_model extends CI_Model {
    ...
    function get_all($limit, $uri) {
        ...
        $result = $this->db->get('siakad_jadwal_ampu', $limit, $uri);
    ...
    function ceknomornull(){
        ...
        $this->db->order_by('id_siakad_jadwal_ampu','ASC');
        $this->db->limit(1);

        $result=$this->db->get('siakad_jadwal_ampu');
    ...
    function get_last(){
        ...
        $this->db->select('*'); //faktur
        $this->db->order_by('id_siakad_jadwal_ampu','DESC');
        $this->db->limit(1);
    ...
    function gettotaldetail($faktur){
        ...
        $this->db->from('siakad_jadwal_ampu');
    ...
    function get_one($id_siakad_jadwal_ampu) {
        $this->db->where('id_siakad_jadwal_ampu', $id_siakad_jadwal_ampu);
        $result = $this->db->get('siakad_jadwal_ampu');
        if ($result->num_rows() == 1) {
            return $result->row_array();
        } else {
    ...
    function savesiakad_jadwal_ampu($data){
        $this->db->insert('siakad_jadwal_ampu',$data);
    }
    function save_detail($data){
        $this->db->insert('siakad_jadwal_ampu_detail',$data);
    }
    function upddel_detail($id_siakad_jadwal_ampu=null) {
    ...
    function updatebyid($id_siakad_jadwal_ampu,$data){
        $this->db->where('id_siakad_jadwal_ampu', $id_siakad_jadwal_ampu);
        $this->db->update('siakad_jadwal_ampu', $data);
    }
    function update($id_siakad_jadwal_ampu) {
        $data = array(
        'id_siakad_jadwal_ampu' => $this->input->post('id_siakad_jadwal_ampu',TRUE),
       'id_siakad_jadwal' => $this->input->post('id_siakad_jadwal', TRUE),
    ...
    function delete($id_siakad_jadwal_ampu) {
        $this->db->where('id_siakad_jadwal_ampu', $id_siakad_jadwal_ampu);
        $this->db->delete('siakad_jadwal_ampu'); 
       
    }
    function delete_detail($id=null) {
        $this->db->where('id_detail', $id);
        $this->db->delete('siakad_jadwal_ampu_detail'); 
    ...
    function get_dropdown_array($value){
        $result = array();
        $array_keys_values = $this->db->query('select id_siakad_jadwal_ampu, '.$value.' from siakad_jadwal_ampu order by id_siakad_jadwal_ampu asc');
        $result[0]="-- Pilih Urutan id_siakad_jadwal_ampu --";
        foreach ($array_keys_values->result() as $row)
        {
            $result[$row->id_siakad_jadwal_ampu]= $row->$value;
        }
        return $result;
    }
