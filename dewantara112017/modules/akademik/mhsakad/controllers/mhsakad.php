...
class mhsakad extends MX_Controller {

    function __construct() {
        parent::__construct();
          
        //Load IgnitedDatatables Library
        $this->load->model('mhs_akademik_model','mhsakadb',TRUE);
        $this->session->set_userdata('lihat','mhs_akademik');
    ...
    public function index() {
        $this->template->set_title('Kelola Akademik Mahasiswa');
        $this->template->add_js('var baseurl="'.base_url().'mhsakad/";','embed');  
        $this->template->load_view('mhs_akademik_view',array(
            'view'=>'Mhs_akademik_data',
            'title'=>'Kelola Data Akademik Mahasiswa',
            'subtitle'=>'Pengelolaan Akademik Mahasiswa',
            'breadcrumb'=>array(
            'Akademik Mahasiswa'),
        ));
    }
    ...
    function __getnewfaktur(){
        ...
        $null=$this->mhsakadb->ceknomornull();
        // print_r($null);
        if($null!=null||!empty($null)){
            ...
            $faktur=$this->mhsakadb->genfaktur();
    ...
    function __submitnomor($data){
       ...
       $this->db->insert('mhs_akademik',$data);
     ...
     function __updatestatproses($faktur){
        ...
        $this->db->update('mhs_akademik',$data);
    }
    ...
    public function getdatatables(){
        if($this->isadmin()==1):
            $this->datatables->select('id,nim,kodepaket,tahunakad,isactive,dateactive,userid,datetime,')
                            ->from('mhs_akademik');
            $this->datatables->add_column('edit',"<div class='btn-group'>
                <a data-toggle='modal' href='#modal-id' data-load-remote='".base_url('mhsakad/getone/$1/')."' data-remote-target='#modal-id .modal-body' class='btn btn-info btn-xs'><i class='fa fa-info-circle'></i> </a>
            ...
            $this->datatables->select('id,nim,kodepaket,tahunakad,isactive,dateactive,userid,datetime,')
                            ->from('mhs_akademik');
            $this->datatables->add_column('edit',"<div class='btn-group'>
                <a data-toggle='modal' href='#modal-id' data-load-remote='".base_url('mhsakad/getone/$1/')."' data-remote-target='#modal-id .modal-body' class='btn btn-info btn-xs'><i class='fa fa-info-circle'></i> </a></div>" , 'id');
            $this->datatables->unset_column('id');
        endif;
    ...
    function enkrip(){
        ...
        // echo $this->session->userdata('mhs_akademik');
    }
    function isadmin(){
    ...
    function forms(){   
        ...
        $this->load->view('mhs_akademik_form_inside');
           
    }

    public function get($id=null){
        if($id!==null){
            echo json_encode($this->mhsakadb->get_one($id));
        }
    }
    function tables(){
        $this->load->view('mhs_akademik_data');
    }

    function getone($id=null){
        if($id!==null){
            $data=$this->mhsakadb->get_one($id);
            $jml=count($data);
            // print_r($jml);
            // print_r($data);
            $div='';
    ...
    public function submit(){
        if ($this->input->post('ajax')){
          if ($this->input->post('id')){
            $this->mhsakadb->update($this->input->post('id'));
          }else{
            //$this->mhsakadb->save();
            $this->mhsakadb->saveas();
          }
    ...
    public function delete(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id'])){
                $this->mhsakadb->delete($this->input->post('id'));
    ...
    public function delete_detail(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id'])){
                $this->mhsakadb->upddel_detail($this->input->post('id'));
                $this->session->set_flashdata('notif','Succeed, Data Has Deleted');
     ...
     public function delete_detailxx(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id'])){
                $this->mhsakadb->delete_detail($this->input->post('id'));
                $this->session->set_flashdata('notif','Succeed, Data Has Deleted');
            }else {
                $this->session->set_flashdata('notif', 'Failed! No Data Deleted');
            }
...
/** Module mhs_akademik Controller **/
