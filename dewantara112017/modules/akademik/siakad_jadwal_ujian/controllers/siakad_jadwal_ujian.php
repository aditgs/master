...
class siakad_jadwal_ujian extends MX_Controller {
    ...
    function __construct() {
        ...
        $this->session->set_userdata('lihat','siakad_jadwal_ujian');
    ...
    public function index() {
        $this->template->set_title('Kelola Siakad_jadwal_ujian');
        $this->template->add_js('var baseurl="'.base_url().'siakad_jadwal_ujian/";','embed');  
        $this->template->load_view('siakad_jadwal_ujian_view',array(
            'view'=>'',
            'title'=>'Kelola Data Siakad_jadwal_ujian',
            'subtitle'=>'Pengelolaan Siakad_jadwal_ujian',
            'breadcrumb'=>array(
            'Siakad_jadwal_ujian'),
        ));
    }
    ...
    function __getnewfaktur(){
        ...
        $null=$this->siakad_jadwal_ujiandb->ceknomornull();
        // print_r($null);
        if($null!=null||!empty($null)){
            ...
            $faktur=$this->siakad_jadwal_ujiandb->genfaktur();
    ...
    function __submitnomor($data){
       ...
       $this->db->insert('siakad_jadwal_ujian',$data);
     ...
     function __updatestatproses($faktur){
        ...
        $this->db->update('siakad_jadwal_ujian',$data);
    }
    ...
    public function getdatatables(){
        if($this->isadmin()==1):
            $this->datatables->select('id_siakad_jadwal_ujian,kode_prodi,id_siakad_jadwal,jenis_ujian,tgl_ujian,ujian_mulai,ujian_selesai,id_siakad_ruang,nip_dosen,')
                            ->from('siakad_jadwal_ujian');
            $this->datatables->add_column('edit',"<div class='btn-group'>
                <a data-toggle='modal' href='#modal-id' data-load-remote='".base_url('siakad_jadwal_ujian/getone/$1/')."' data-remote-target='#modal-id .modal-body' class='btn btn-info btn-xs'><i class='fa fa-info-circle'></i> </a>
            ...
            $this->datatables->select('id_siakad_jadwal_ujian,kode_prodi,id_siakad_jadwal,jenis_ujian,tgl_ujian,ujian_mulai,ujian_selesai,id_siakad_ruang,nip_dosen,')
                            ->from('siakad_jadwal_ujian');
            $this->datatables->add_column('edit',"<div class='btn-group'>
                <a data-toggle='modal' href='#modal-id' data-load-remote='".base_url('siakad_jadwal_ujian/getone/$1/')."' data-remote-target='#modal-id .modal-body' class='btn btn-info btn-xs'><i class='fa fa-info-circle'></i> </a></div>" , 'id_siakad_jadwal_ujian');
            $this->datatables->unset_column('id_siakad_jadwal_ujian');
        endif;
    ...
    function enkrip(){
        ...
        // echo $this->session->userdata('siakad_jadwal_ujian');
    }
    function isadmin(){
    ...
    function forms(){   
        ...
        $this->load->view('siakad_jadwal_ujian_form_inside');
           
    }

    public function get($id_siakad_jadwal_ujian=null){
        if($id_siakad_jadwal_ujian!==null){
            echo json_encode($this->siakad_jadwal_ujiandb->get_one($id_siakad_jadwal_ujian));
        }
    }
    function tables(){
        $this->load->view('siakad_jadwal_ujian_data');
    }

    function getone($id=null){
        if($id!==null){
            $data=$this->siakad_jadwal_ujiandb->get_one($id);
            $jml=count($data);
            // print_r($jml);
            // print_r($data);
            $div='';
    ...
    public function submit(){
        if ($this->input->post('ajax')){
          if ($this->input->post('id_siakad_jadwal_ujian')){
            $this->siakad_jadwal_ujiandb->update($this->input->post('id_siakad_jadwal_ujian'));
          }else{
            //$this->siakad_jadwal_ujiandb->save();
            $this->siakad_jadwal_ujiandb->saveas();
          }
    ...
    public function delete(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id'])){
                $this->siakad_jadwal_ujiandb->delete($this->input->post('id'));
    ...
    public function delete_detail(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id_siakad_jadwal_ujian'])){
                $this->siakad_jadwal_ujiandb->upddel_detail($this->input->post('id_siakad_jadwal_ujian'));
                $this->session->set_flashdata('notif','Succeed, Data Has Deleted');
     ...
     public function delete_detailxx(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id_siakad_jadwal_ujian'])){
                $this->siakad_jadwal_ujiandb->delete_detail($this->input->post('id_siakad_jadwal_ujian'));
                $this->session->set_flashdata('notif','Succeed, Data Has Deleted');
            }else {
                $this->session->set_flashdata('notif', 'Failed! No Data Deleted');
            }
...
/** Module siakad_jadwal_ujian Controller **/
