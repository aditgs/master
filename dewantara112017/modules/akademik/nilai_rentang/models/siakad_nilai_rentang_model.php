...
class Siakad_nilai_rentang_model extends CI_Model {
    ...
    function get_all($limit, $uri) {
        ...
        $result = $this->db->get('siakad_nilai_rentang', $limit, $uri);
    ...
    function ceknomornull(){
        ...
        $this->db->order_by('id_siakad_nilai_rentang','ASC');
        $this->db->limit(1);

        $result=$this->db->get('siakad_nilai_rentang');
    ...
    function get_last(){
        ...
        $this->db->select('*'); //faktur
        $this->db->order_by('id_siakad_nilai_rentang','DESC');
        $this->db->limit(1);
    ...
    function gettotaldetail($faktur){
        ...
        $this->db->from('siakad_nilai_rentang');
    ...
    function get_one($id_siakad_nilai_rentang) {
        $this->db->where('id_siakad_nilai_rentang', $id_siakad_nilai_rentang);
        $result = $this->db->get('siakad_nilai_rentang');
        if ($result->num_rows() == 1) {
            return $result->row_array();
        } else {
    ...
    function __save(){
       ...
       $data = array(
            ...
            'maks_sks' => $this->input->post('maks_sks', TRUE),
           
            'ip_bobot_awal' => $this->input->post('ip_bobot_awal', TRUE),
           
            'ip_bobot_akhir' => $this->input->post('ip_bobot_akhir', TRUE),
    ...
    function savesiakad_nilai_rentang($data){
        $this->db->insert('siakad_nilai_rentang',$data);
    }
    function save_detail($data){
        $this->db->insert('siakad_nilai_rentang_detail',$data);
    }
    function upddel_detail($id_siakad_nilai_rentang=null) {
    ...
    function updatebyid($id_siakad_nilai_rentang,$data){
        $this->db->where('id_siakad_nilai_rentang', $id_siakad_nilai_rentang);
        $this->db->update('siakad_nilai_rentang', $data);
    }
    function update($id_siakad_nilai_rentang) {
        $data = array(
        'id_siakad_nilai_rentang' => $this->input->post('id_siakad_nilai_rentang',TRUE),
       'kode_prodi' => $this->input->post('kode_prodi', TRUE),
    ...
    function delete($id_siakad_nilai_rentang) {
        $this->db->where('id_siakad_nilai_rentang', $id_siakad_nilai_rentang);
        $this->db->delete('siakad_nilai_rentang'); 
       
    }
    function delete_detail($id=null) {
        $this->db->where('id_detail', $id);
        $this->db->delete('siakad_nilai_rentang_detail'); 
    ...
    function get_dropdown_array($value){
        $result = array();
        $array_keys_values = $this->db->query('select id_siakad_nilai_rentang, '.$value.' from siakad_nilai_rentang order by id_siakad_nilai_rentang asc');
        $result[0]="-- Pilih Urutan id_siakad_nilai_rentang --";
        foreach ($array_keys_values->result() as $row)
        {
            $result[$row->id_siakad_nilai_rentang]= $row->$value;
        }
        return $result;
    }
