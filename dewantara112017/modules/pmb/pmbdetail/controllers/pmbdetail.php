...
class pmbdetail extends MX_Controller {

    function __construct() {
        parent::__construct();
          
        //Load IgnitedDatatables Library
        $this->load->model('pmb_detail_model','pmb_detaildb',TRUE);
        $this->session->set_userdata('lihat','pmb_detail');
    ...
    public function index() {
        $this->template->set_title('Kelola Pmb_detail');
        $this->template->add_js('var baseurl="'.base_url().'pmb_detail/";','embed');  
        $this->template->load_view('pmb_detail_view',array(
            'view'=>'',
            'title'=>'Kelola Data Pmb_detail',
            'subtitle'=>'Pengelolaan Pmb_detail',
            'breadcrumb'=>array(
            'Pmb_detail'),
        ));
    }
    ...
    function __getnewfaktur(){
        ...
        $null=$this->pmb_detaildb->ceknomornull();
        // print_r($null);
        if($null!=null||!empty($null)){
            ...
            $faktur=$this->pmb_detaildb->genfaktur();
    ...
    function __submitnomor($data){
       ...
       $this->db->insert('pmb_detail',$data);
     ...
     function __updatestatproses($faktur){
        ...
        $this->db->update('pmb_detail',$data);
    }
    ...
    public function getdatatables(){
        if($this->isadmin()==1):
            $this->datatables->select('id,kodepmb,keterangan,gel_id,jalur_id,date_start,date_end,date_seleksi_start,date_seleksi_end,time_start,time_end,pengawas,ruang_test,kodetarifdaftar,userid,datetime,')
                            ->from('pmb_detail');
            $this->datatables->add_column('edit',"<div class='btn-group'>
                <a data-toggle='modal' href='#modal-id' data-load-remote='".base_url('pmb_detail/getone/$1/')."' data-remote-target='#modal-id .modal-body' class='btn btn-info btn-xs'><i class='fa fa-info-circle'></i> </a>
            ...
            $this->datatables->select('id,kodepmb,keterangan,gel_id,jalur_id,date_start,date_end,date_seleksi_start,date_seleksi_end,time_start,time_end,pengawas,ruang_test,kodetarifdaftar,userid,datetime,')
                            ->from('pmb_detail');
            $this->datatables->add_column('edit',"<div class='btn-group'>
                <a data-toggle='modal' href='#modal-id' data-load-remote='".base_url('pmb_detail/getone/$1/')."' data-remote-target='#modal-id .modal-body' class='btn btn-info btn-xs'><i class='fa fa-info-circle'></i> </a></div>" , 'id');
            $this->datatables->unset_column('id');
        endif;
    ...
    function enkrip(){
        ...
        // echo $this->session->userdata('pmb_detail');
    }
    function isadmin(){
    ...
    function forms(){   
        ...
        $this->load->view('pmb_detail_form_inside');
           
    }

    public function get($id=null){
        if($id!==null){
            echo json_encode($this->pmb_detaildb->get_one($id));
        }
    }
    function tables(){
        $this->load->view('pmb_detail_data');
    }

    function getone($id=null){
        if($id!==null){
            $data=$this->pmb_detaildb->get_one($id);
            $jml=count($data);
            // print_r($jml);
            // print_r($data);
            $div='';
    ...
    public function submit(){
        if ($this->input->post('ajax')){
          if ($this->input->post('id')){
            $this->pmb_detaildb->update($this->input->post('id'));
          }else{
            //$this->pmb_detaildb->save();
            $this->pmb_detaildb->saveas();
          }
    ...
    public function delete(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id'])){
                $this->pmb_detaildb->delete($this->input->post('id'));
    ...
    public function delete_detail(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id'])){
                $this->pmb_detaildb->upddel_detail($this->input->post('id'));
                $this->session->set_flashdata('notif','Succeed, Data Has Deleted');
     ...
     public function delete_detailxx(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id'])){
                $this->pmb_detaildb->delete_detail($this->input->post('id'));
                $this->session->set_flashdata('notif','Succeed, Data Has Deleted');
            }else {
                $this->session->set_flashdata('notif', 'Failed! No Data Deleted');
            }
