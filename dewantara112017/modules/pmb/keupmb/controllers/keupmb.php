...
class Keupmb extends MX_Controller {
    ...
    public function index() {
        $this->template->set_title('Kelola Uang Pendaftaran');
        $this->template->add_js('var baseurl="'.base_url().'keupmb/";','embed');  
        $this->template->load_view('siakad_keu_pendaftaran_view',array(
            'view'=>'datakeupmb',
            'title'=>'Kelola Data Uang Pendaftaran',
            'subtitle'=>'Pengelolaan Uang Pendaftaran',
            'breadcrumb'=>array(
    ...
    function __getnewfaktur(){
        ...
        $null=$this->keupmbdb->ceknomornull();
        // print_r($null);
        if($null!=null||!empty($null)){
            ...
            $faktur=$this->keupmbdb->genfaktur();
    ...
    public function getdatatables(){
        if($this->isadmin()==1):
            ...
            $this->datatables->add_column('edit',"<div class='btn-group'>
                <a data-toggle='modal' href='#modal-id' data-load-remote='".base_url('keupmb/getone/$1/')."' data-remote-target='#modal-id .modal-body' class='btn btn-info btn-xs'><i class='fa fa-info-circle'></i> </a>
            ...
            $this->datatables->select('id_siakad_keu_pendaftaran,thn_akademik,nm_pendaftaran,biaya_pendaftaran,')
                            ->from('siakad_keu_pendaftaran');
            $this->datatables->add_column('edit',"<div class='btn-group'>
                <a data-toggle='modal' href='#modal-id' data-load-remote='".base_url('keupmb/getone/$1/')."' data-remote-target='#modal-id .modal-body' class='btn btn-info btn-xs'><i class='fa fa-info-circle'></i> </a></div>" , 'id_siakad_keu_pendaftaran');
            $this->datatables->unset_column('id_siakad_keu_pendaftaran');
        endif;
    ...
    public function get($id_siakad_keu_pendaftaran=null){
        if($id_siakad_keu_pendaftaran!==null){
            echo json_encode($this->keupmbdb->get_one($id_siakad_keu_pendaftaran));
        }
    }
    function tables(){
    ...
    public function submit(){
        if ($this->input->post('ajax')){
          if ($this->input->post('id_siakad_keu_pendaftaran')){
            $this->keupmbdb->update($this->input->post('id_siakad_keu_pendaftaran'));
          }else{
            //$this->keupmbdb->save();
            $this->keupmbdb->saveas();
          }
    ...
    public function delete(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id'])){
                $this->keupmbdb->delete($this->input->post('id'));
    ...
    public function delete_detail(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id_siakad_keu_pendaftaran'])){
                $this->keupmbdb->upddel_detail($this->input->post('id_siakad_keu_pendaftaran'));
                $this->session->set_flashdata('notif','Succeed, Data Has Deleted');
     ...
     public function delete_detailxx(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id_siakad_keu_pendaftaran'])){
                $this->keupmbdb->delete_detail($this->input->post('id_siakad_keu_pendaftaran'));
                $this->session->set_flashdata('notif','Succeed, Data Has Deleted');
            }else {
                $this->session->set_flashdata('notif', 'Failed! No Data Deleted');
            }
