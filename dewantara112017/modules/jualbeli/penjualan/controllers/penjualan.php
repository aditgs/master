...
class penjualan extends MX_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('penjualan_model','jualdb',TRUE);
        $this->session->set_userdata('lihat','penjualan');
    ...
    public function index() {
        $this->template->set_title('Kelola Penjualan');
        $this->template->add_js('var baseurl="'.base_url().'penjualan/";','embed');  
        ...
        $this->template->load_view('penjualan_view',array(
            'view'=>'form_penjualan',
            'title'=>'Kelola Data Penjualan',
            'subtitle'=>'Pengelolaan Penjualan',
            'breadcrumb'=>array(
            'Penjualan'),
        ));
    }
    ...
    public function getdatatables(){
        if($this->isadmin()==1):
            $this->datatables->select('id,Faktur,FakturAsli,Tgl,Jthtmp,Term,Customer,NmCustomer,cAccPiutang,Gudang,cKeterangan,Salesman,Mitra,NmMitra,GudangMT,NmGudangMT,Kandang,NmKandang,PersPPn,PersDiscount,PersDiscount2,PersDiscount3,SubTotal,PPn,Discount,Discount2,Discount3,Pembulatan,Ongkos,Total,Tunai,Kas,Bank,Piutang,Lunas,Retur,DiscLunas,Biaya,Sisa,SLunas,SRetur,SDiscLunas,Status,cNoJrn,lVoid,lPosted,isdeleted,datedeleted,islocked,isactive,User,userid,datetime,Time,')
                            ->from('penjualan');
            $this->datatables->add_column('edit',"<div class='btn-group'>
                <a data-toggle='modal' href='#modal-id' data-load-remote='".base_url('penjualan/getone/$1/')."' data-remote-target='#modal-id .modal-body' class='btn btn-info btn-xs'><i class='fa fa-info-circle'></i> </a>
            ...
            $this->datatables->select('id,Faktur,FakturAsli,Tgl,Jthtmp,Term,Customer,NmCustomer,cAccPiutang,Gudang,cKeterangan,Salesman,Mitra,NmMitra,GudangMT,NmGudangMT,Kandang,NmKandang,PersPPn,PersDiscount,PersDiscount2,PersDiscount3,SubTotal,PPn,Discount,Discount2,Discount3,Pembulatan,Ongkos,Total,Tunai,Kas,Bank,Piutang,Lunas,Retur,DiscLunas,Biaya,Sisa,SLunas,SRetur,SDiscLunas,Status,cNoJrn,lVoid,lPosted,isdeleted,datedeleted,islocked,isactive,User,userid,datetime,Time,')
                            ->from('penjualan');
            $this->datatables->add_column('edit',"<div class='btn-group'>
                <a data-toggle='modal' href='#modal-id' data-load-remote='".base_url('penjualan/getone/$1/')."' data-remote-target='#modal-id .modal-body' class='btn btn-info btn-xs'><i class='fa fa-info-circle'></i> </a></div>" , 'id');
            $this->datatables->unset_column('id');
        endif;
    ...
    function enkrip(){
        ...
        // echo $this->session->userdata('penjualan');
    }
    function isadmin(){
    ...
    function forms(){   
        ...
        $this->load->view('penjualan_form_inside');
           
    }

    public function get($id=null){
        if($id!==null){
            echo json_encode($this->jualdb->get_one($id));
        }
    }
    function tables(){
        $this->load->view('penjualan_data');
    }

    function getone($id=null){
        if($id!==null){
            $data=$this->jualdb->get_one($id);
            $jml=count($data);
            // print_r($jml);
            // print_r($data);
            $div='';
    ...
    public function submit(){
        if ($this->input->post('ajax')){
          if ($this->input->post('id')){
            $this->jualdb->update($this->input->post('id'));
          }else{
            //$this->jualdb->save();
            $this->jualdb->saveas();
          }
    ...
    public function delete(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id'])){
                $this->jualdb->delete($this->input->post('id'));
    ...
    public function delete_detail(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id'])){
                $this->jualdb->upddel_detail($this->input->post('id'));
                $this->session->set_flashdata('notif','Succeed, Data Has Deleted');
     ...
     public function delete_detailxx(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id'])){
                $this->jualdb->delete_detail($this->input->post('id'));
                $this->session->set_flashdata('notif','Succeed, Data Has Deleted');
            }else {
                $this->session->set_flashdata('notif', 'Failed! No Data Deleted');
            }
