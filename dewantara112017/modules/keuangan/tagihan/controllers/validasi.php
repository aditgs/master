...
class validasi extends MX_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('validasi_model','valdb',TRUE);
        $this->load->model('tagihan_model','tagihdb',TRUE);
        $this->load->model('tarif/tarif_model','tarifdb',TRUE);
        $this->load->model('bayar_detail_model','detaildb',TRUE);
    ...
    public function index() {
        ...
        $this->template->add_js('var baseurl="'.base_url().'tagihan/validasi/";','embed');  
        $this->template->add_js('modules/bayar.js');
        $this->template->add_css('forms.css');

        $this->template->load_view('validasi_view',array(
            'default'=>array('kode'=>$this->valdb->genfaktur()),
            'opt_mhs'=>$this->valdb->get_dropdown_mhs(),
            'opt_inv'=>$this->valdb->getdropinvoice(),
            'view'=>'datavalidasi',
            'form_view'=>'formvalidasi',
            'title'=>'Kelola Data Validasi',
            'title_form'=>'Validasi Tagihan',
    ...
    function updstatus(){
        ...
        $this->valdb->updatestatus($id,$status);
    }
    ...
    public function getinvoice($mhs=null){
            ...
            $this->datatables->add_column('edit',"<div class='btn-group'>
                <a data-toggle='modal' href='#modal-id' data-load-remote='".base_url('validasi/getone/$1/')."' data-remote-target='#modal-id .modal-body' class='btn btn-info btn-xs'><i class='fa fa-info-circle'></i> </a>
    ...
    public function gettagihdetail(){
        ...
        $idmhs=$this->input->post('mhs');
            ...
            $this->datatables->edit_column('isvalidated','$1',"getstatus(id,valdb)");
            ...
            $this->datatables->unset_column('nim,isactive,kodetarif');
        echo $this->datatables->generate();
    }
    public function getdatatables(){
        if($this->isadmin()==1):
            ...
            $this->datatables->select('id,kode,invoice,itembayar,tanggal,bank,refbank,tglbank,totalbayar,sisabayar,totaltagihan,sisatagihan,isvalidasi,tglvalidasi,isactive,islocked,isdeleted,datedeleted,userid,datetime,')
                            ->from('bayar');
            $this->datatables->add_column('edit',"<div class='btn-group'>
                <a data-toggle='modal' href='#modal-id' data-load-remote='".base_url('validasi/getone/$1/')."' data-remote-target='#modal-id .modal-body' class='btn btn-info btn-xs'><i class='fa fa-info-circle'></i> </a></div>" , 'id');
            $this->datatables->unset_column('id');
        endif;
        echo $this->datatables->generate();
    }
    ...
    function forms(){   
         // $this->template->add_js('modules/bayar.js');
        $html=$this->load->view('formvalidasi',array(
             'default'=>array('kode'=>$this->valdb->genfaktur()),
            'opt_mhs'=>$this->valdb->get_dropdown_mhs(),
        ),TRUE);
    ...
    public function get($id=null){
        if($id!==null){
            echo json_encode($this->valdb->get_one($id));
        }
    }
    ...
    public function submit(){
         if($this->__formvalidation()===TRUE):
            ...
            // print_r($bayar);
            $tagihan=$this->valdb->gettagihanbykode($this->input->post('kode', TRUE));
            $data = array(
            ...
            if ($this->input->post('ajax')){
              if ($this->input->post('id')){
                $this->valdb->update($this->input->post('id'));
              }else{
                //$this->valdb->save();
                // $this->valdb->saveas();
                $this->valdb->savebayar($data);
                $this->valdb->savedetailbatch($dx);
              }
    ...
    public function delete(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id'])){
                $this->valdb->delete($this->input->post('id'));
    ...
    public function delete_detail(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id'])){
                $this->valdb->upddel_detail($this->input->post('id'));
                $this->session->set_flashdata('notif','Succeed, Data Has Deleted');
     ...
     public function delete_detailxx(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id'])){
                $this->valdb->delete_detail($this->input->post('id'));
                $this->session->set_flashdata('notif','Succeed, Data Has Deleted');
            }else {
                $this->session->set_flashdata('notif', 'Failed! No Data Deleted');
            }
