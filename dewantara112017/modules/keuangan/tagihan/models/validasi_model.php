...
class Validasi_model extends CI_Model {
    ...
    function getstatus($id){
        $this->db->select('id,status_detail as status')->from('007-view-validasi')->where('id',$id);
        $result=$this->db->get();
        if($result->num_rows()==1){
            return $result->row_array();
        }else{
    ...
    function genfaktur(){
        ...
        // print_r($last);
        if(!empty($last)):
            $faktur=genfaktur($last['kode'],"VAL",3);//diganti sesuai faktur/kode transaksi
        else:
            $faktur="VAL".date('ym')."0001";//diganti sesuai faktur/kode transaksi
        endif;
        return ($faktur);
    }
    function get_one($id) {
